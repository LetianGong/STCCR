...
def geohash2array(geohash_code):
    alphabet = '0123456789abcdefghijklmnopqrstuvwxyz'
    result = []
    for i in geohash_code:
        result.append(alphabet.find(i))
    return result


def category2vector(word_list, tokenizer, bert_model):
    tokens = tokenizer.encode_plus(text=word_list, return_tensors='pt')
    output = bert_model(**tokens)
    return output.last_hidden_state[:, 0, :].squeeze(-1).detach().numpy()
